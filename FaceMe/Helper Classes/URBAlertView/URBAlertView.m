...
//viral changed due to square pop up and change dimention
...
#define kURBAlertPadding 8.0
#define kURBAlertButtonPadding 2.0
#define kURBAlertButtonHeight 54.0
#define kURBAlertButtonOffset 43.0
#define kURBAlertTextFieldHeight 29.0
...
- (void)initialize {
//	_backgroundColor = [UIColor colorWithWhite:0.22 alpha:1.0];//viral
    _backgroundColor = [UIColor whiteColor];

	_backgroundGradation = 0.1;
//	_strokeColor = [UIColor colorWithWhite:0.8 alpha:1.000];//viral
    _strokeColor = [UIColor whiteColor];

//	_strokeWidth = 3.0;
//	_cornerRadius = 6.0;//viral
    
    _strokeWidth = 0.0;//viral
    _cornerRadius = 0.0;//viral
	
	_titleFont = [UIFont boldSystemFontOfSize:15.0];
//	_titleColor = [UIColor whiteColor];//viral
    _titleColor = [UIColor darkGrayColor];//viral
//    _titleColor = [UIColor blackColor];
//    _titleColor = [APP_DELEGATE colorWithHexString:yellow_color];
//    _titleColor = [UIColor clearColor];


//	_titleShadowColor = [UIColor blackColor];//viral
    _titleShadowColor = [UIColor clearColor];

	_titleShadowOffset = CGSizeMake(0.0, -1.0);
	
	_messageFont = [UIFont systemFontOfSize:14.0];
//	_messageColor = [UIColor whiteColor];//viral
//    _messageColor = [UIColor darkGrayColor];//viral
    _messageColor = [UIColor blackColor];


//	_messageShadowColor = [UIColor blackColor];//viral
    _messageShadowColor = [UIColor clearColor];

	_messageShadowOffset = CGSizeMake(0.0, -1.0);
	
//	_buttonBackgroundColor = [UIColor colorWithWhite:0.35 alpha:1.0];//viral
//    _buttonBackgroundColor = [UIColor blueColor];
//    _buttonBackgroundColor = [APP_DELEGATE colorWithHexString:yellow_color];//viral
...
//	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deviceOrientationChanged:) name:UIDeviceOrientationDidChangeNotification object:nil];//viral
	
	// register for keyboard notifications
//	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillShow:) name:UIKeyboardWillShowNotification object:nil];//viral
//	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillHide:) name:UIKeyboardWillHideNotification object:nil];//viral
...
//	button.titleLabel.font = [UIFont boldSystemFontOfSize:16.0];//viral
    button.titleLabel.font = [UIFont boldSystemFontOfSize:18.0];
//    button.titleLabel.font = [UIFont fontWithName:SinkinSansBoldFont size:16];//viral change button text color

//    [button setTitleColor:[APP_DELEGATE colorWithHexString:yellow_color] forState:UIControlStateNormal];
...
- (void)addTextFieldWithPlaceholder:(NSString *)placeholder secure:(BOOL)secure {
	...
	field.delegate = self;
    self.focusedTextField.autocorrectionType = NO;
...
//            CGAffineTransform rotate = CGAffineTransformRotate(transform, 50.0 * M_PI / 180.0);
//            CGAffineTransform translate = CGAffineTransformTranslate(transform, 00.0, -screenSize.height / 2.0 - CGRectGetWidth(self.bounds));
//            self.transform = translate;//viral
...
//                self.transform = translate;//viral
    ...
    else if (animation == URBAlertAnimationTopToBottom)//viral
    {
        if (show)
        {
            [self showOverlay:YES];
...
//            CGAffineTransform rotate = CGAffineTransformRotate(transform, 50.0 * M_PI / 180.0);//viral
//            CGAffineTransform translate = CGAffineTransformTranslate(transform, 00.0, -screenSize.height / 2.0 - CGRectGetWidth(self.bounds));//top to bottom
            CGAffineTransform translate1 = CGAffineTransformMakeTranslation(0, 568);//viral bottom to top

//            self.transform = translate;
            self.transform = translate1;//viral
...
//    UIImageView *beloved = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"logo_yellow_small.png"]];//beloved-log17N.png
//    UIImageView *beloved = [[UIImageView alloc] init];//beloved-log17N.png
//    beloved.frame=CGRectMake(layoutFrame.size.width/2-25,-25 , 50,50); //75, -25, 130, 60
//    [beloved setImage:[UIImage imageNamed:@"logo_circle.png"]];
//    //    beloved.frame = CGRectMake(self.contentView.frame.size.height/2, 0, 130, 60);
//    beloved.layer.borderColor = [UIColor whiteColor].CGColor;
//    beloved.backgroundColor = [UIColor whiteColor];
//    beloved.contentMode = UIViewContentModeScaleAspectFit;
//    beloved.clipsToBounds =YES;
//    beloved.layer.borderWidth = 2.0;
//    beloved.layer.cornerRadius = 25.0;
////    beloved.frame = layoutFrame;
//    [self.contentView addSubview:beloved]; //temp 16-4 //viral
    
    UIImageView *beloved = [[UIImageView alloc] init];//beloved-log17N.png
    beloved.frame=CGRectMake(layoutFrame.size.width/2-25,0 , 50,25); //75, -25, 130, 60
    [beloved setImage:[UIImage imageNamed:@"fivelogo.png"]];
//    [beloved setImage:[UIImage imageNamed:@"5fiveHeader.png"]];
    //    beloved.frame = CGRectMake(self.contentView.frame.size.height/2, 0, 130, 60);
    beloved.layer.borderColor = [UIColor whiteColor].CGColor;
    beloved.backgroundColor = [UIColor clearColor];
    beloved.contentMode = UIViewContentModeScaleAspectFit;
//    beloved.clipsToBounds =YES;
//    beloved.layer.borderWidth = 2.0;
//    beloved.layer.cornerRadius = 25.0;
    //    beloved.frame = layoutFrame;
//    [self.contentView addSubview:beloved];
	...
	if (self.title.length > 0) {
		if ([self.title respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
			CGRect titleBounds = [self.title boundingRectWithSize:CGSizeMake(CGRectGetWidth(textFrame), MAXFLOAT) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName:self.titleFont}
														  context:nil];
			titleHeight = titleBounds.size.height;
		}
		else {
			CGSize titleSize = [self.title sizeWithFont:self.titleFont constrainedToSize:CGSizeMake(CGRectGetWidth(textFrame), MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping];
	...
	if (self.message.length > 0) {
		if ([self.title respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
			CGRect messageBounds = [self.message boundingRectWithSize:CGSizeMake(CGRectGetWidth(textFrame), MAXFLOAT) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName:self.messageFont} context:nil];
			messageHeight = messageBounds.size.height;
		}
		else {
			CGSize messageSize = [self.message sizeWithFont:self.messageFont constrainedToSize:CGSizeMake(CGRectGetWidth(textFrame), MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping];
	...
	CGFloat buttonRegionOffsetY = ([self.buttons count] > 2) ? (kURBAlertButtonHeight * [self.buttons count] + kURBAlertButtonPadding * ([self.buttons count] - 1) + 25) : kURBAlertButtonOffset;
	...
	layout.buttonRegionRect = CGRectMake(kURBAlertFrameInset, CGRectGetHeight(self.bounds) - buttonRegionHeight - kURBAlertFrameInset,  CGRectGetWidth(self.bounds) - kURBAlertFrameInset * 2.0, buttonRegionOffsetY);
...
//        CGFloat buttonWidth = (totalButtons > 2) ? CGRectGetWidth(layout.buttonRect) : (CGRectGetWidth(layout.buttonRect) - kURBAlertButtonPadding-3.5 * ((CGFloat)count - 1.0)) / (CGFloat)count;
        CGFloat buttonWidth = (totalButtons > 2) ? CGRectGetWidth(layout.buttonRect) : (CGRectGetWidth(layout.buttonRect)+4 ) / (CGFloat)count;
...
//                xOffset = CGRectGetMinX(layout.buttonRect)-2 + (kURBAlertButtonPadding-2 + buttonWidth+4) * (CGFloat)i; ////13-5
                xOffset =  buttonWidth* (CGFloat)i; ////13-5
            ...
            {
                frame = CGRectIntegral(CGRectMake(xOffset, yOffset+5.0, buttonWidth+0.0, CGRectGetHeight(layout.buttonRect)-10)); //5.0 //13-5
            }
            else
            {
                frame = CGRectIntegral(CGRectMake(xOffset, yOffset+6.0, buttonWidth+4.0, CGRectGetHeight(layout.buttonRect)-10)); //5.0
            }
...
//	CGContextSetLineWidth(context, 1.0f);//viral
    CGContextSetLineWidth(context, 0.0f);//viral 17-4
...
//	keylinePath.lineWidth = 1.0;//viral remove seperator line
    keylinePath.lineWidth = 0.0;
//    keylinePath.lineWidth = 0.5;

	
//	CGContextTranslateCTM(context, 0, CGRectGetMinY(hatchFrame) - 2.0);//viral
    CGContextTranslateCTM(context, 0, CGRectGetMinY(hatchFrame) + 7.0);

	[self.darkKeylineColor setStroke];
	[keylinePath stroke];
//	CGContextTranslateCTM(context, 0, 1.0);//viral
...
//	UIColor *gradientInner = [UIColor colorWithRed: 0 green: 0 blue: 0 alpha: 0.1];
    ...
    UIColor *gradientOuter = [UIColor colorWithRed: 0 green: 0 blue: 0 alpha: 0.5];
    UIColor *gradientInner = [UIColor colorWithRed: 0 green: 0 blue: 0 alpha: 0.5];//viral for removing round background shadow
...
//		_strokeColor = [UIColor colorWithWhite:0.2 alpha:1.0];//viral
        _strokeColor = [UIColor whiteColor];
...
//    // colors 255,213,132 //beloved
//    UIColor *buttonBgGradientColorTop = [UIColor colorWithRed:255.0/255 green:213.0/255 blue:132.0/255 alpha:1];
//    UIColor *buttonBgGradientColorBottom = [UIColor colorWithRed:255.0/255 green:213.0/255 blue:132.0/255 alpha:1];
//    UIColor *buttonOuterGradientColor = [UIColor colorWithRed:255.0/255 green:213.0/255 blue:132.0/255 alpha:1];
//    UIColor *buttonOuterGradientColor2 = [UIColor colorWithRed:255.0/255 green:213.0/255 blue:132.0/255 alpha:1];
//    UIColor *buttonInnerGradientColor = [UIColor colorWithRed:255.0/255 green:213.0/255 blue:132.0/255 alpha:1];
//    UIColor *buttonInnerGradientColor2 = [UIColor colorWithRed:255.0/255 green:213.0/255 blue:132.0/255 alpha:1];
    
    // colors 255,213,132 //LOV
    UIColor *buttonBgGradientColorTop = [UIColor clearColor];
    UIColor *buttonBgGradientColorBottom = [UIColor clearColor];
    UIColor *buttonOuterGradientColor = [UIColor clearColor];
    UIColor *buttonOuterGradientColor2 = [UIColor clearColor];
    UIColor *buttonInnerGradientColor = [UIColor clearColor];
    UIColor *buttonInnerGradientColor2 = [UIColor clearColor];//viral
...
//	CGFloat buttonShadowBlurRadius = 2;
    ...
    UIColor *buttonShadow = [UIColor whiteColor];
    CGSize buttonShadowOffset = CGSizeMake(0.0, 0.0);
...
//								0);
//	CGContextRestoreGState(context);//viral
	
	// outer drawing
//	CGRect buttonOuterRect = CGRectInset(frame, 4.0, 4.0);
    CGRect buttonOuterRect = CGRectInset(frame, 0.0, 0.0);//viral
	...
	// inner drawing
//	CGRect buttonInnerRect = CGRectInset(frame, 6.0, 6.0);//viral
    CGRect buttonInnerRect = CGRectInset(frame, 0.0, 0.0);
...
//	// colors //original button background
//	UIColor *buttonBgGradientColorTop = [UIColor colorWithWhite:0.3 alpha:1.0];
//	UIColor *buttonBgGradientColorBottom = [UIColor colorWithWhite:0.25 alpha:1.0];
...
//	UIColor *buttonInnerGradientColor2 = [self.backgroundColor adjustBrightness:1.05];
    ...
    UIColor *buttonOuterGradientColor = [UIColor clearColor];
    UIColor *buttonOuterGradientColor2 = [UIColor clearColor];
//    UIColor *buttonInnerGradientColor = [UIColor lightGrayColor];
//    UIColor *buttonInnerGradientColor2 = [UIColor lightGrayColor];//viral
    
    UIColor *buttonInnerGradientColor = [APP_DELEGATE colorWithHexString:@"d3d3d3"];//light gray color
    UIColor *buttonInnerGradientColor2 = [APP_DELEGATE colorWithHexString:@"d3d3d3"];//viral
	...
	// shadows
	UIColor *buttonShadow = [UIColor clearColor];
	CGSize buttonShadowOffset = CGSizeMake(0.1, -0.1);
	CGFloat buttonShadowBlurRadius = 2;
...
//	CGContextRestoreGState(context);//viral
	...
	CGContextBeginTransparencyLayer(context, NULL);
	[buttonOuterPath addClip];
	CGContextDrawLinearGradient(context, buttonOuterGradient, CGPointMake(CGRectGetMidX(buttonOuterRect), CGRectGetMinY(buttonOuterRect)), CGPointMake(CGRectGetMidX(buttonOuterRect), CGRectGetMaxY(buttonOuterRect)), 0);
	CGContextEndTransparencyLayer(context);
	CGContextRestoreGState(context);
